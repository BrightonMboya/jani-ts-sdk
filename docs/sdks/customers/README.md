...
async function run() {
  const result = await janiPayments.customers.listAddresses({
    customerId: "<id>",
  });
...
```typescript
import { JaniPaymentsCore } from "jani-payments/core.js";
import { customersListAddresses } from "jani-payments/funcs/customersListAddresses.js";
...
async function run() {
  const res = await customersListAddresses(janiPayments, {
    customerId: "<id>",
  });
