...
async function run() {
  const result = await janiPayments.subscriptions.pause({
    subscriptionId: "<id>",
    requestBody: {
      effectiveFrom: "next_billing_period",
...
```typescript
import { JaniPaymentsCore } from "jani-payments/core.js";
import { subscriptionsPause } from "jani-payments/funcs/subscriptionsPause.js";
...
async function run() {
  const res = await subscriptionsPause(janiPayments, {
    subscriptionId: "<id>",
    requestBody: {
      effectiveFrom: "next_billing_period",
      onResume: "start_new_billing_period",
